...
| **Validation MCD** (règles Merise) | `api/services/merise_rules.py` + `mcd_service.validate_mcd()` | Appel HTTP `POST /validate` |
...
- **Ajouter une règle Merise** : l’écrire dans `api/services/merise_rules.py`, puis l’utiliser dans `api/services/mcd_service.py`. Flutter n’a rien à changer si elle s’applique à la validation ou au passage MCD→MLD.
